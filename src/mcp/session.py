...
class MCPSessionManager:
    ...
    async def close(self) -> None:
        ...
        Uses try-finally to ensure cleanup even if persistence fails.
        """
        persist_errors = []
        ...
        try:
            ...
            # Always clear sessions, even if persistence failed
            session_count = len(self._sessions)
            self._sessions.clear()

            if persist_errors:
                logger.warning(
                    "MCPSessionManager closed with errors",
                    total_sessions=session_count,
                    failed_persists=len(persist_errors),
                    errors=persist_errors
                )
            else:
                logger.info(
                    "MCPSessionManager closed",
