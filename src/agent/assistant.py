...
class AIAssistant:
    ...
    def __init__(self, config: AgentConfig = None) -> None:
        ...
        self._rate_limiter = RateLimiter(RateLimitConfig(
            enabled=getattr(self.config, 'rate_limit_enabled', False),
            requests_per_minute=getattr(self.config, 'rate_limit_rpm', 60),
        ))
    ...
    async def process_question(
        ...
        with tracer.start_as_current_span("process_question") as span:
            ...
            try:
                ...
                logger.info("Processing question", question=question[:100], chat_id=chat_id, model=model)
            ...
            try:
                ...
                async for update in self.agent.run_stream(question, thread=thread):
                    if hasattr(update, 'text') and update.text:
                        # Capture author_name from SDK's AgentResponseUpdate
                        author = getattr(update, 'author_name', None)
                        yield StreamChunk(text=update.text, done=False, author=author)
