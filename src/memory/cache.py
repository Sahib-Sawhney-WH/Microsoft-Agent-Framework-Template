...
class RedisCache:
    ...
    async def _ensure_connected(self) -> bool:
        ...
        try:
            ...
            #
            # SECURITY NOTE: We decode without signature verification here because:
            # 1. The token comes directly from DefaultAzureCredential (trusted source)
            # 2. We only extract the OID claim for Redis authentication
            # 3. The actual authentication is performed by Redis using the full token
            # 4. If the token were tampered with, Redis auth would fail anyway
            #
            # For tokens from EXTERNAL sources, always verify signatures.
            try:
                # First, validate the token has the expected structure
                token_parts = token_response.token.split(".")
                if len(token_parts) != 3:
                    raise ValueError("Invalid JWT structure - expected 3 parts")

                decoded = jwt.decode(
                    token_response.token,
                    options={"verify_signature": False}  # Safe: token from trusted Azure SDK
                )

                # Validate expected claims exist
                if "oid" not in decoded and "sub" not in decoded:
                    logger.warning("Token missing expected claims (oid/sub)")

                username = decoded.get("oid", decoded.get("sub", ""))
