...
class ChatHistoryManager:
    ...
    def __init__(
        ...
        self._running = False
        self._start_lock = asyncio.Lock()  # Guard against double-start
    ...
    async def start_background_persist(self) -> None:
        ...
        Uses a lock to prevent double-start in concurrent scenarios.
        """
        ...
        async with self._start_lock:
            ...
            if self._running:
                logger.debug("Background persist task already running")
                return

            # Check if task exists but is done (crashed)
            if self._persist_task is not None:
                if self._persist_task.done():
                    # Task completed or crashed, check for exception
                    try:
                        exc = self._persist_task.exception()
                        if exc:
                            logger.warning(
                                "Previous persist task crashed, restarting",
                                error=str(exc)
                            )
                    except asyncio.CancelledError:
                        pass  # Task was cancelled, OK to restart
                    self._persist_task = None
                else:
                    # Task still running but _running is False - fix state
                    self._running = True
                    logger.debug("Background persist task state repaired")
                    return

            self._running = True
            self._persist_task = asyncio.create_task(
                self._background_persist_loop(),
                name="background_persist_loop"
            )
