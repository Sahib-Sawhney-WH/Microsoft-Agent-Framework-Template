...
**Error Rate:**
```promql
rate(ai_assistant_errors_total[5m])
```

**P95 Latency:**
```promql
histogram_quantile(0.95, rate(ai_assistant_request_latency_milliseconds_bucket[5m]))
```

**Token Usage:**
```promql
rate(ai_assistant_tokens_total[5m])
```
...
- [Architecture Overview](../architecture/index.md) — Observability architecture
- [Docker Deployment](../deployment/docker.md) — Jaeger setup with Docker Compose
- [Production Checklist](../deployment/production-checklist.md) — Observability requirements
